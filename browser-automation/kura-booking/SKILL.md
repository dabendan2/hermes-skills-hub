...
## 瀏覽器工具手動預訂 (Manual Tool Workflow)
當腳本失效或需要手動介入時，請遵循以下精確步驟：
1. **登入**：`browser_navigate` 至 `/login`，使用 `browser_type` 填寫電子郵件與密碼，點擊「登入」。
2. **進入分店**：導航至 `https://e-pai-ke.com/shop/[ID]`。
3. **開啟預約**：點擊 `a.bookbtn:has-text("預約")`。
4. **關鍵等待 (10s)**：開啟彈窗後，**必須靜置 10 秒**。此時背景正在載入該店的剩餘名額數據，若太早點擊日期，時段將無法載入。
5. **選取日期**：點擊 `#reserve_date` 開啟日曆，點擊目標日期數字的 `<a>` 標籤。
6. **選取時段**：時段 ID 格式為 `#hour_HH_min_MM` (例如 `#hour_18_min_30`)。
    - **注意**：時段可能是 `button` 或 `input[type="button"]`。若 `browser_click` 失敗，請使用 `browser_console` 執行 `document.querySelector('#hour_...').click()`。
7. **選取人數**：先點擊 `.div-select` (下拉選單觸發器)，再點擊對應的 `li[data-sub="N"]`。
8. **確認預約**：連續點擊兩次 `#orderOK`。
